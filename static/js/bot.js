...
function bot_get_postflop_bet () {
  ...
  var VERDICT = "";
  var STRAIGHT_FLUSH = is_straight_flush(P);
  var FOUR_OF_A_KIND = is_four_of_a_kind(P);
  var FULL_HOUSE = is_full_house(P);
  var FLUSH = is_flush(P);
  var STRAIGHT = is_straight(P);
  var THREE_OF_A_KIND = is_three_of_a_kind(P);
  var TWO_PAIR = is_two_pair(P);
  var ONE_PAIR = is_one_pair(P);
  var FLUSH_DRAW = 0;
...
function internal_setup () {
  ...
  BET_LEVEL = internal_get_bet_level(current_bet); // feed function data we calc here so we don't gotta doubl do it!..
